...
Template.createTeam.events({
	...
	'submit .create-channel__content'(e, instance) {
		...
		Meteor.call(isPrivate ? 'createPrivateGroup' : 'createTeam', name, instance.selectedUsers.get().map((user) => user.username), readOnly, {}, extraData, avatar, function(err, result) {
			if (err) {
